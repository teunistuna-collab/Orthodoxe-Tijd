  ...
  return <>
    ...
    <section className="feasts-page bg-parchment py-12 text-ink sm:py-16"><div className={CONTENT}>
      <div className="overflow-hidden border border-gold/45 bg-[#f7edda] shadow-[0_24px_55px_rgba(56,31,14,.16)]">
        ...
        <section className="border-b border-gold/30 px-6 py-9 sm:px-10 sm:py-12"><div className="text-center"><p className="ot-label">Pascha en de twaalf grote feesten</p><h2 className="font-display mt-2 text-4xl text-ink">De grote feesten</h2></div><div className="mlr-lijst mt-6 lg:hidden">{DERTIEN.map(f => { const item=lijst.find(x=>x.f.id===f.id)!; return <MobileListRow key={f.id} onClick={()=>setOpen(f.id)} links={<><b>{item.datum.getUTCDate()}</b><small>{MAANDEN_KORT[item.datum.getUTCMonth()]}</small><small>{item.datum.getUTCFullYear()}</small></>} titel={f.naam} onder={f.offset!==undefined?'Beweeglijk':`Vast · ${formatMd(f.md!)}`}/>})}</div><div className="feast-tiles mt-8 grid gap-px overflow-hidden border border-gold/30 bg-gold/30 max-lg:hidden md:grid-cols-2 xl:grid-cols-3">{DERTIEN.map(f => { const item=lijst.find(x=>x.f.id===f.id)!; return <button key={f.id} onClick={()=>setOpen(f.id)} className="ornate-card feast-tile group flex flex-col items-center text-center"><span className="ornate-medallion feast-medallion">✣</span><span className="ornate-side-ornaments" aria-hidden="true">❦ <b>✣</b> ❦</span><span className="feast-meta">{f.offset!==undefined?'Beweeglijk':`Vast · ${formatMd(f.md!)}`}</span><h3>{f.naam}</h3><p>{formatLang(item.datum)}</p></button>})}</div></section>

        <section className="grid border-b border-gold/30 lg:grid-cols-2"><div className="px-7 py-9 sm:px-10 lg:border-r lg:border-gold/30"><p className="ot-label">Één jaar — twee ritmes</p><h3 className="font-display mt-2 text-3xl">Vaste feesten</h3><p className="ot-tekst mt-3 text-sm leading-6 text-ink-soft">Deze gedachtenissen keren ieder kerkelijk jaar terug op dezelfde kerkelijke datum en worden door de jaarcyclus gedragen.</p><a href="#jaar" className="btn-pill mt-5">Ontdek de jaarcyclus →</a></div><div className="bg-[#efe3cb]/55 px-7 py-9 sm:px-10"><p className="ot-label">Rond Pascha</p><h3 className="font-display mt-2 text-3xl">Beweeglijke feesten</h3><p className="ot-tekst mt-3 text-sm leading-6 text-ink-soft">De datum van Pascha bepaalt onder meer de Grote Week, Hemelvaart en Pinksteren. Deze data komen uit dezelfde centrale Pascha-berekening als de kalender.</p><a href="#pascha" className="btn-pill mt-5">Ontdek de Paschacyclus →</a></div></section>
